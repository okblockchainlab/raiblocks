...
typedef bool (*produce_unsigned_tx_t)(
  const std::string& link,
  const std::string& previous,
  const std::string& representative,
  const std::string& balance_s,
  std::string& utx);
typedef bool (*sign_transaction_t)(
  const std::string& utx,
  const std::string& prv,
  const std::string& net_type,
  std::string& stx);
typedef bool (*test_init_t)(const std::string& test_prv1, const std::string& test_account2);
typedef bool (*commit_transaction_t)(const std::string& stx, const std::string& net_type, std::string& result_str);
...
public:
  ...
  ASSERT_FALSE(cwd.empty());
    ...
    ASSERT_TRUE(testInit(test_genesis_prv, test_account2));
  }
...
TEST_F(OKWalletTest, produceUnsignedTx) {
  ...
  const char* previous = "04270D7F11C4B2B472F2854C5A59F2A7E84226CE9ED799DE75744BD7D85FC9D9";
  const char* representative = "xrb_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtdo";
  const char* balance = "340282366920938463463374607431768201455";

  std::string utx;
  ASSERT_TRUE(produceUnsignedTx(test_account2, previous, representative, balance, utx));
...
TEST_F(OKWalletTest, signTransaction) {
  ...
  std::string stx;
  ASSERT_TRUE(signTransaction(utx, test_genesis_prv, "testnet", stx));
